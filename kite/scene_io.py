...
class GMTSAR(SceneIO):
    ...
    def _getPRMFile(self, path):
        if not op.isdir(path):
            path = op.dirname(path)
        prm_files = glob.glob(op.join(path, "*.PRM"))
        if len(prm_files) == 0:
            self._log.warning(
                "Could not find PRM files. Primary and secondary times will be set to deafult"
            )
        elif len(prm_files) == 1:
            self._log.warning(
                "Missing 1 PRM file (2 requiered). Primary and secondary times will be set to deafult"
            )
            # raise ImportError("Could not find PRM file (*.PRM)")
        self._log.debug("Found PRM file(s): %s", prm_files)
        return prm_files
    ...
    def read(self, path, **kwargs):
        ...
        try:
            ...
            container.phi = 0.0
        # Get datetimes
        if len(self._getPRMFile(path)) == 2:
            times = [None] * 2
            for i, file in enumerate(self._getPRMFile(path)):

                with open(str(file), mode="r", encoding="us-ascii") as prm:

                    for line in prm:
                        if "SC_clock_start" in line:
                            jul_day = line.split()[2]
                            times[i] = datetime(
                                int(jul_day[:4]), 1, 1, tzinfo=timezone.utc
                            ) + timedelta(days=float(jul_day[4:]))

            times.sort()
            container.meta.time_primary = times[0].timestamp()
            container.meta.time_secondary = times[1].timestamp()
...
class ARIA(SceneIO):
    ...
    def read(self, folder, **kwargs):
        ...
        t_secondary, t_primary = c.meta.scene_id.split("_")
        c.meta.time_primary = datetime(
            *time.strptime(t_primary, "%Y%m%d")[:6]
        ).timestamp()
        c.meta.time_secondary = datetime(
            *time.strptime(t_secondary, "%Y%m%d")[:6]
        ).timestamp()
