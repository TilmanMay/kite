...
class Meta(Object):
    ...
    time_primary = Timestamp.T(
        default=1481116161.930574, help="Timestamp for primary acquisition"
    )
    time_secondary = Timestamp.T(
        default=1482239325.482, help="Timestamp for secondary acquisition"
    )
    ...
    def time_separation(self):
        """
        :getter: Absolute time difference between ``time_primary``
                 and ``time_secondary``
        :type: timedelta
        """
        return dt.fromtimestamp(self.time_secondary) - dt.fromtimestamp(
            self.time_primary
        )
...
class BaseScene(object):
    ...
    def __add__(self, other, copy_obj=True):
        ...
        tmin = (
            ret.meta.time_primary
            if ret.meta.time_primary < other.meta.time_primary
            else other.meta.time_primary
        )

        tmax = (
            ret.meta.time_secondary
            if ret.meta.time_secondary > other.meta.time_secondary
            else other.meta.time_secondary
        )

        ret.meta.time_primary = tmin
        ret.meta.time_secondary = tmax
        return ret
